...
def convertObj2Track(obj:OrderedDict, dis: OrderedDict)->list:
    res = []
...
class CentroidTracker2():
    ...
    def register(self, centroid, full_data):
        # when registering an object we use the next available object
        # ID to store the centroid
        self.objects[self.nextObjectID] = (centroid, full_data)
        self.disappeared[self.nextObjectID] = 0
        self.nextObjectID += 1
    ...
    def update(self, pred):
        ...
        if len(self.objects) == 0:
            for i in range(0, len(inputCentroids)):
                self.register(inputCentroids[i], pred[i])
        ...
        # centroids
            ...
            for (row, col) in zip(rows, cols):
                ...
                # counter
                objectID = objectIDs[row]
                self.objects[objectID] = (inputCentroids[col], pred[col])
                self.disappeared[objectID] = 0
                # indicate that we have examined each of the row and
                # column indexes, respectively
                usedRows.add(row)
